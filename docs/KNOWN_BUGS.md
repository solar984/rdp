...
- `sockaddr_cmp` returns an uninitialized local value when both endpoints use the same unsupported address family.  IPv4, family 6, and the historical legacy family have defined paths.
...
The default hash constructor checks the address family, allocation size, bucket count, and allocation result before storing the new table.
...
- The historical `net_connect` and `net_shutdown` wrapper uses a single unsynchronized process global endpoint.  `net_shutdown` assumes that endpoint exists even when its connection argument is null.
...
- The Mac `uevent_create` wrapper reports success even when semaphore creation fails.  The POSIX implementation keeps this event behavior for source compatibility.
...
- `discard_log_append` lazily opens a single process global file, never closes it, and does not synchronize initialization or writes.
...
IPv4 UDP, threads, synchronization, timing, and wake datagrams are implemented on Windows and POSIX.  POSIX reports the obsolete legacy transport and physical serial backend as unavailable.
