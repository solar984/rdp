...
{
    static const char greeting[] = "hello from the rdplib API";
    ...
    if (result == RDPLIB_OK)
    {
        result = rdplib_endpoint_create(runtime, &endpoint, 0, 1, RDPLIB_USE_CRC);
    }
    if (result == RDPLIB_OK)
    {
        result = rdplib_connect(endpoint, &connection, host, port);
    }
    if (result == RDPLIB_OK)
    {
    ...
    if (result == RDPLIB_OK)
    {
        result = rdplib_connection_send(connection, greeting, (uint32_t)strlen(greeting), 0, RDPLIB_SEND_RELIABLE);
    ...
    while (result == RDPLIB_OK && !received_reply && !saw_fin && !saw_disconnect)
    {
        rdplib_message_t *message;
        int process_result = rdplib_endpoint_process(endpoint, 5000);
        ...
        while ((message = rdplib_connection_pop_message(connection)) != NULL)
        {
            ...
            {
                printf("reply: %.*s\n", (int)rdplib_message_size(message), (const char *)rdplib_message_data(message));
                received_reply = 1;
            }
            saw_fin = saw_fin || rdplib_message_has_fin(message);
            saw_disconnect = saw_disconnect || rdplib_message_is_disconnect(message);

            // Pop transfers ownership; release on every path after consuming the bytes.
    ...
    if (!received_reply && (saw_fin || saw_disconnect) && result == RDPLIB_OK)
    {
        fprintf(stderr, "the connection ended before a reply arrived\n");
        result = RDPLIB_ERROR_NOT_USABLE;
    }
    ...
    if (connection)
    {
        ...
        // A long running application keeps processing this endpoint naturally.  This short example gives the nonblocking close time to finish before destroying it.
        if (close_started)
        {
    ...
    if (runtime && rdplib_runtime_destroy(runtime) != RDPLIB_OK)
    {
        result = 1;
    }
    return result == RDPLIB_OK && received_reply ? 0 : 1;
}
