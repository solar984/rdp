...
static void send_probe(rdp_t *owner, const uint8_t destination[16])
{
    ...
    assert(rdplib_platform_send_datagram(owner->ipv4_socket, marker, sizeof(marker), destination) == (int32_t)sizeof(marker));
}
...
int main(void)
{
    ...
    assert(owner->icmp_receive_socket == -1);
    assert(owner->icmp_probe_socket == -1);
    ...
    assert(closed_connection->transmit.disconnect_reason == RDP_DISCONNECT_REASON_ICMP);
    assert(closed_connection->receive.last_icmp_type == 3);
    assert(closed_connection->receive.last_icmp_code == 3);
    ...
    assert(closed_connection->receive.icmp_count == 1);
